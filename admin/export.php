...
if ($person=="" AND $team=="")   {
    ...
    $query = mysqli_query($con, "SELECT * FROM users WHERE branch IN ($branch)");
    $array1=array();
      ...
      while($row = mysqli_fetch_assoc($query)){
       ...
       $query3 = mysqli_query($con, "SELECT * FROM branch WHERE branch_id IN ($branch)");
      ...
      if (empty($array2)){
          ...
          header( "refresh:5;url=generate.php" );
    echo $array2;
    ?>
...
<a href="#">You will be redirected in 5s</a>
</div>
</div>
...
<script>
  ...
  gtag('config', 'UA-23581568-13');
    ...
    <?php
       ...
       $query1 = mysqli_query($con, "SELECT * FROM prospects WHERE user_id IN ($array2) AND status IN ($status) AND date2<='$to' AND date2>='$from'");
 ...
 if ($num > 0)
 {
  $output .= '
                   ...
                   <tr><td colspan="7" style="background-color:yellow;">Sales Report for Branch(s): '.$array4.'</td></tr>
